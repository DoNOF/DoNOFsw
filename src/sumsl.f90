...
!old if (iv(1) - 2) 30, 40, 50
!old
!old 30   nf = iv(nfcall)
!old  call calcf(n, x, nf, f, uiparm, urparm)
!old  if (nf <= 0) iv(toobig) = 1
!old  go to 20
!old
!old 40   call calcg(n, x, iv(nfgcal), v(g1), uiparm, urparm)
!old  go to 20
!old
!old 50   if (iv(1) /= 14) then
!old        return
!old      end if
...
!change
 if (iv(1) < 2) THEN
  ...
  call calcg(n, x, iv(nfgcal), v(g1), uiparm, urparm)
  go to 20
 else
  if (iv(1) /= 14) then
   return
